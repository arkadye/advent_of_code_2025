...
namespace advent_testcase_internal
{
	...
	inline std::string as_std_string(std::integral auto in) { return std::to_string(in); }

	template <typename T> requires std::constructible_from<std::string,T>
	inline std::string as_std_string(T in) { return std::string{ in }; }

	inline std::string as_std_string(std::string in) { return in; }
}

verification_test make_test(std::string name, TestFunc func, Dummy);

inline verification_test make_test(std::string name, TestFunc func, auto result)
{
	using advent_testcase_internal::as_std_string;
	return advent_testcase_internal::make_test(std::move(name), std::move(func), as_std_string(std::move(result)));
}

inline verification_test make_test(std::string name, TestFuncWithArg func, auto result, auto arg)
{
	using advent_testcase_internal::as_std_string;
	return advent_testcase_internal::make_test(std::move(name), std::move(func), as_std_string(result), as_std_string(arg));
}
inline verification_test make_test(std::string name, TestFuncWithArg func, Dummy, auto arg)
{
	using advent_testcase_internal::as_std_string;
	return advent_testcase_internal::make_test(std::move(name), std::move(func), dummy, as_std_string(arg));
}
